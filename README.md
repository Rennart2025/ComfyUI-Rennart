...
Указав один цвет на панели выбора, нода позволяет:
Работать по Цветовому кругу Иттена: автоматически генерировать классические гармонии — комплементарные, триады, аналоговые, тетрады и split-комплементарные сочетания на основе законов цветоведения.
Настраивать ключи (Color Keys): управлять  тональностью изображения, выстраивая палитры в высоком ключе (High Key — светлые, воздушные тона), низком ключе (Low Key — глубокие, тёмные, драматичные оттенки) или задавать ключевые доминантные цвета.

Формировать палитры: отдавать HEX-коды для дальнейшей передачи акцентных и базовых цветов в ноды генерации, тонирования и стилизации (в частности, для JSON-промптов Ideogram-4).
